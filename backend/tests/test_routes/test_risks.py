...
async def test_add_and_list_risks(db_conn):
    ...
    cur = await db_conn.execute(
        "SELECT id, name, category, source FROM app.assessment_risks WHERE assessment_id = %s",
        (aid,),
    )
...
async def test_patch_likelihood_impact(db_conn):
    ...
    cur = await db_conn.execute(
        "SELECT inherent_likelihood, inherent_impact FROM app.assessment_risks WHERE id = %s",
        (rid,),
    )
